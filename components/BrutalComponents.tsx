...
export const BrutalBox: React.FC<BrutalBoxProps> = ({ children, className = "", title }) => (
  <div className={`border-4 border-black bg-white shadow-hard p-6 relative ${className}`}>
    {title && (
      <div className="absolute -top-5 left-4 bg-brutal-yellow border-4 border-black px-2 py-1 transform -rotate-1 z-10">
        <span className="font-display font-bold uppercase text-lg tracking-wider">{title}</span>
      </div>
    )}
    {children}
  </div>
...
export const BrutalInput: React.FC<BrutalInputProps> = ({ label, required, className = "", ...props }) => (
  <div className="flex flex-col gap-2 mb-4">
    <label className="font-mono font-bold uppercase text-sm border-l-4 border-black pl-2 flex justify-between">
      <span>{label}</span>
      {required && <span className="text-brutal-red text-xs font-black">* REQ</span>}
    </label>
    <input
      className={`
...
export const BrutalTextArea: React.FC<BrutalTextAreaProps> = ({ label, required, className = "", ...props }) => (
  <div className="flex flex-col gap-2 mb-4">
    <label className="font-mono font-bold uppercase text-sm border-l-4 border-black pl-2 flex justify-between">
      <span>{label}</span>
      {required && <span className="text-brutal-red text-xs font-black">* REQ</span>}
...
export const BrutalCheckbox: React.FC<CheckboxProps> = ({ label, checked, onChange }) => (
  <div 
    onClick={onChange}
    className={`
    ...
    `}
  >
    <div className={`w-6 h-6 border-4 border-current flex items-center justify-center`}>
